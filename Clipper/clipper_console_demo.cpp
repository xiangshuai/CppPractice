...
public:
  ...
  bool SaveToFile(char * filename, double scale = 1.0, int margin = 10)
  {
    ...
    rec.left = (long long)((double)rec.left * scale);
    rec.top = (long long)((double)rec.top * scale);
    rec.right = (long long)((double)rec.right * scale);
    rec.bottom = (long long)((double)rec.bottom * scale);
    long long offsetX = -rec.left + margin;
    long long offsetY = -rec.top + margin;
...
{
  if ((val < 0)) return (long long)(val - 0.5); else return (long long)(val + 0.5);
}
//------------------------------------------------------------------------------
